...
def create_generators(train_val_split=0.8, n_hist=4, n_pred=1):
    ds = read_xarray_dataset(args.dataseries_directory)

    ds_len = len(ds["time"])
    sample_length = n_hist + n_pred
    ...
    train_gen = SAFDataGenerator(ds=train_ds, n_hist=n_hist, n_pred=n_pred)
    val_gen = SAFDataGenerator(ds=val_ds, n_hist=n_hist, n_pred=n_pred)
...
class SAFDataGenerator:
    def __init__(self, **kwargs):
        ...
        self.sample_length = self.n_hist + self.n_pred
    ...
    def __getitem__(self, idx):
        ...
        x = torch.from_numpy(data[:self.n_hist]).contiguous()
        y = torch.from_numpy(data[self.n_hist:]).contiguous()
        ...
        assert x.shape == (
            ...
            args.input_size[0],
            args.input_size[1],
        ), f"x shape is {x.shape}, should be ({self.n_hist}, {args.input_size[0]}, {args.input_size[1]})"
        assert y.shape == (
            self.n_pred,
            args.input_size[0],
            args.input_size[1],
        ), f"y shape is {y.shape}, should be ({self.n_pred}, {args.input_size[0]}, {args.input_size[1]})"
